...
.gold-card .value { color: #ffd700; } /* Gold color for backtest results */
...
.stButton > button { background: transparent !important; color: #7b7bff !important; border: 1px solid #7b7bff !important; border-radius: 6px !important; font-family: 'Courier New', monospace !important; font-size: 0.85rem !important; padding: 0.6rem 2rem !important; width: 100% !important; position: relative !important; overflow: hidden !important; z-index: 1 !important; transition: all 0.3s ease !important; }
.stButton > button::after { content: ""; position: absolute; top: 50%; left: 50%; width: 150px; height: 150px; background: rgba(123, 123, 255, 0.4); border-radius: 50%; transform: translate(-50%, -50%) scale(0); opacity: 0; z-index: -1; }
.stButton > button:hover { color: #ffffff !important; box-shadow: 0 0 15px rgba(123, 123, 255, 0.3) !important; }
.stButton > button:hover::after { animation: ripple-effect 1.2s infinite ease-out; }
@keyframes ripple-effect { 0% { transform: translate(-50%, -50%) scale(0); opacity: 0.8; } 100% { transform: translate(-50%, -50%) scale(3); opacity: 0; } }
</style>
...
with header_left:
    ...
    <div class="title-block">
        <div class="ssh-tag">SSH v0.2 · BIST AI 2Y BACKTEST</div>
        <h1>Stock Squeeze Hunter</h1>
        <p>Detects squeeze patterns and backtests them for 15% profit within 21 days using 2-year historical data.</p>
    </div>
    """, unsafe_allow_html=True)
...
with b2:
    if st.button("🎯 Percentile — Price Range"):
        ...
        st.rerun()
with b3:
    start_scan = st.button("🚀 Start Scan + Backtest")

# --- SCAN & PROGRESS BAR SECTION ---
if start_scan:
    ...
    def progress_callback(ticker, i, total):
        status_text.markdown(f'<div class="status-bar">Scanning & Backtesting {ticker} · {i}/{total}</div>', unsafe_allow_html=True)
        progress_bar.progress(i / total)

    # Execute the scanning function
    now, last30, failed, total_scanned, global_stats = run_scan(progress_callback=progress_callback, mode=st.session_state.scan_mode)
    st.session_state.results = (now, last30, failed, total_scanned, global_stats)
    
    progress_bar.empty()
    status_text.markdown('<div class="status-bar" style="color: #7b7bff;">✓ Scan & Backtest complete</div>', unsafe_allow_html=True)

# --- RESULTS DISPLAY ---
if st.session_state.results:
    now, last30, failed, total_scanned, global_stats = st.session_state.results

    # Calculate and display global backtest statistics
    total_signals = global_stats["signals"]
    total_wins = global_stats["wins"]
    
    overall_wr = f"%{(total_wins / total_signals * 100):.1f}" if total_signals > 0 else "%0"
    avg_breakout = f"+%{(global_stats['sum_breakout'] / total_signals):.1f}" if total_signals > 0 else "%0"
    avg_days = f"{(global_stats['sum_days'] / total_wins):.1f} Days" if total_wins > 0 else "0 Days"

    st.markdown('<div class="section-header" style="color: #ffd700;">🧠 STRATEGY BACKTEST RESULTS (Last 2 Years)</div>', unsafe_allow_html=True)
    g1, g2, g3 = st.columns(3)
    with g1:
        st.markdown(f'<div class="metric-card gold-card"><div class="value">{overall_wr}</div><div class="label">Overall Win Rate (≥15% in 3 Weeks)</div></div>', unsafe_allow_html=True)
    with g2:
        st.markdown(f'<div class="metric-card gold-card"><div class="value">{avg_breakout}</div><div class="label">Avg Breakout in 21 Days</div></div>', unsafe_allow_html=True)
    with g3:
        st.markdown(f'<div class="metric-card gold-card"><div class="value">{avg_days}</div><div class="label">Avg Time to Target</div></div>', unsafe_allow_html=True)
    ...
    if now:
        df_now = pd.DataFrame(now).sort_values("Win Rate", ascending=False)
        df_now["Chart"] = df_now["Ticker"].apply(lambda t: f"https://www.tradingview.com/chart/?symbol=BIST:{t}")

        # Button to open all charts via JavaScript
        btn_col, _ = st.columns([1, 4])
        with btn_col:
            if st.button("🌐 Open All in TradingView"):
                js_code = "<script>"
    ...
    # Squeezed in Last 30 Days Table
    st.markdown('<div class="section-header">📅 Squeezed in Last 30 Days</div>', unsafe_allow_html=True)
    if last30:
        df_last30 = pd.DataFrame(last30).sort_values("Win Rate", ascending=False)
